...
c	end subroutine
	...
	subroutine nufft2dIIapp(nj,plan,c,U,V,xxsub,ns,kflag,r,S)
	implicit none
	integer  r,i,j,k,nj,ns,kflag,num
	integer mm
	integer xxsub(nj)
	complex*16 M(r,ns*ns),N(ns*ns,r),S(nj),c(ns*ns),U(nj,r),V(r,ns*ns)
        complex*16 cc(ns,ns),c2(ns*ns)
	...
	if (kflag .lt. 0) then
		...
		allocate(c1(mm,mm))
		c1=cc(1:mm,1:mm)
		cc(1:mm,1:mm)=cc(mm+1:ns,mm+1:ns)
		cc(mm+1:ns,mm+1:ns)=c1
		c1=cc(1:mm,mm+1:ns)
		cc(1:mm,mm+1:ns)=cc(mm+1:ns,1:mm)
		cc(mm+1:ns,1:mm)=c1
		c2=reshape(cc,(/ns*ns/))
        endif
        M=0
